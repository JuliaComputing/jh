...
3. **Command Structure**:
   ...
   - `jh package`: Package search and info (GraphQL primary, REST fallback via `/packages/info`; supports filtering by registry)
...
- `fetchRegistries` in `registries.go` is used by `listRegistries`, `packageSearchCmd`, and `packageInfoCmd` to resolve registry names to IDs (for GraphQL) and names (for REST fallback)
